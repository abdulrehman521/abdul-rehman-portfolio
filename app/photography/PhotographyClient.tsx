...
export default function PhotographyClient({ categories }: Props) {
  const [selectedCategory, setSelectedCategory] = useState(categories[0]);
  ...
  useEffect(() => {
    const fetchImages = async () => {
      ...
      if (error) {
        ...
        setImagesByCategory(prev => ({ ...prev, [selectedCategory]: [] }));
      } else {
        setImagesByCategory(prev => ({ ...prev, [selectedCategory]: data.map(img => img.url) }));
      }
      setLoading(false);
    };
